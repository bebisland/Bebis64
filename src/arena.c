...
Result arena_create(Arena *arena, size_t capacity) {
  if (!arena)
    return result_err(RESULT_ERR_NULL, "arena_create: NULL pointer");

  arena->buffer = (unsigned char *)malloc(capacity);
  if (!arena->buffer) {
    log_error("arena_create: allocation failed (%zu bytes)", capacity);
    return result_err(RESULT_ERR_ALLOC, "arena_create: malloc failed");
  }
...
void *arena_alloc_aligned(Arena *arena, size_t size, size_t alignment) {
  ...
  if (aligned + size > arena->capacity) {
    log_error("arena_alloc: out of memory (requested %zu, remaining %zu)",
